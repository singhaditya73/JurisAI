...
def run_pretraining(skip_if_exists: bool = False) -> str:
    ...
    if skip_if_exists and Path(output_dir).exists():
        ...
        if adapter_files:
            console.print(
                f"[yellow]⚠ Pretrained adapter found at {output_dir}[/yellow]"
            )
            console.print(
                "[yellow]  Skipping pretraining (use --no-skip to retrain)[/yellow]"
            )
            return output_dir
    ...
    pretrain_data_path = str(
        PROJECT_ROOT / "data" / "processed" / "pretrain" / "train.jsonl"
    )
    ...
    console.print(
        f"  Batch size: {pretrain_args.get('per_device_train_batch_size', 1)}"
    )
    console.print(
        f"  Grad accumulation: {pretrain_args.get('gradient_accumulation_steps', 8)}"
    )
    console.print(
        f"  Effective batch: {pretrain_args.get('per_device_train_batch_size', 1) * pretrain_args.get('gradient_accumulation_steps', 8)}"
    )
    ...
    console.print(
        f"  Samples/sec: {trainer_stats.metrics.get('train_samples_per_second', 0):.2f}"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="JurisAI Stage 1: Continual Pretraining"
    )
    parser.add_argument(
        "--skip-if-exists",
        action="store_true",
